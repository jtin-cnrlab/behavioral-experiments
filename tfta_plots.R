...
dprime_hsection_plot <- ggplot(data = dprime_hsection,
    ...
    scale_y_continuous(limits = c(2.35, 3.85)) +
    labs(title = "d' by Half-Section by Day", y = "d'",
...
rt_bysection_boxplot <- ggplot(data = rt_box) +
    geom_boxplot(aes(x = section, y = rt, fill = as.character(day),
                     group = interaction(as.character(day), section)), alpha = 0.9) +
...
ggsave(file.path("plots-training","rt_boxplots.png"), width = 7, height = 6)

#### TEST PLOT: RT BY TEST ####
pre_vs_post <- test %>%
    mutate(talker = ifelse(talker <= 4, "setA", "setB"),
           trained_t = ifelse(talker == talker_trained, "trained", "untrained")) %>%
...
pre_vs_post$trained_t <- factor(pre_vs_post$trained_t, levels = c("untrained", "trained"),
                                labels = c("Untrained Talkers", "Trained Talkers"))
pre_vs_post_plot <- ggplot(data = pre_vs_post, aes(x = single_mixed, y = mean_rt, color = test,
                                                   group = test)) +
    geom_line(size = 1) +
    geom_point(size = 3.5) +
    scale_y_continuous(limits = c(625, 850), breaks = c(seq.int(600, 900, 50))) +
    facet_grid(carrier ~ trained_t) +
    labs(title = "Pre- vs. Post-test, \nTrained Vowels Only",
         x = "", y = "Mean RT (ms)",
         caption = paste0("setA n = ", setA_n, "\nsetB n = ", setB_n)) +
    scale_color_viridis(discrete = TRUE) +
    theme(strip.placement = "outside",
          strip.background = element_blank(),
          strip.text = element_text(size = 12),
...
pre_vs_post_plot
ggsave(file.path("plots-test", "pre_vs_post.png"), width = 5, height = 5.5)
...
tx_trainedv <- tx_master %>%
    ...
    mutate(talker = ifelse(talker <= 4, "setA", "setB"),
           trained_t = ifelse(talker == talker_trained, "trained", "untrained"),
           word_vowel = ifelse(word_vowel %in% c("u", "o"), "pairA", "pairB"),
           trained_v = ifelse(word_vowel == vowel_trained, "trained", "untrained")) %>%
...
tx_trainedv_plot <- ggplot(data = tx_trainedv, aes(x = part, y = score_percent*100)) +
    ...
    geom_boxplot(aes(fill = trained_t), position = position_dodge(.9)) +
    geom_point(data = filter(tx_trainedv, trained_t == "Untrained Talkers"),
               aes(group = participant), alpha = 0.5, position = position_nudge(x = -.1)) +
    geom_segment(data = tx_trainedv_segments, aes(x = x, y = y*100, xend = xend, yend = yend*100,
                                                  color = participant), alpha = 0.5) +
...
tx_bar_plot <- ggplot(data = tx_bar, aes(x = part, y = pct*100)) +
    #geom_hline(aes(yintercept = 50), linetype = "dashed", color = "gray80") +
    ...
    labs(title = "Transcription Accuracy", subtitle = "96 words",
         x = "", y = "Percent Correct",
         caption = paste0("setA n = 3", "\nsetB n = 1")) +
    scale_fill_viridis(discrete = TRUE, begin = 0.25, end = 0.75) +
    theme(strip.placement = "outside",
          ...
          legend.title = element_blank())
tx_bar_plot
ggsave(file.path("plots-transcription", "trainved_vs_untrained_vowels.png"), width = 6.25, height = 6.5)
